...
int main()
{
	...
	// sigfillset(&sigset); //every signal is in the set, we can substract from it later
	sigprocmask(SIG_BLOCK, &sigset, NULL); // signals in sigset will be blocked
	// parameters, 
		// 1. - how: SIG_BLOCK, SIG_UNBLOCK, SIG_SETMASK -   ;
		// 2. - changes the signalset to this if it is not NULL,
		// 3. - if it is not NULL, the formerly used set is stored here
	...
	if (child > 0)
	{
		...
		// then the other signals in the sigset is also received (LIFO)
