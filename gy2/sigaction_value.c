...
timer_t t_id;
// when using a timer, a -lrt swich is required when compiling
// signal handler
...
int main()
{
	...
	sigact.sa_flags = SA_SIGINFO; // we need to set the siginfo flag, so the os will send the extra data as well
	sigaction(SIGTERM, &sigact, NULL);
	// Parameters: 
		// 1st - signal
		// 2nd - pointer, os stores the extra data in this variable
...
// Output
//// Send a SIGTERM by the timer in 1 sec
//// Signal with number 15 has arrived
//// It was sent by a timer
//// Additional data: 5
//// ---------------------------------
//// Parent (PID 1777) waits for a signal from child (PID 1778)
//// Child process (PID 1778), will send a SIGTERM to parent
//// Signal with number 15 has arrived
//// Process (PID number 1778) sent the signal (calling kill)
//// ---------------------------------
//// Signal with number 15 has arrived
//// Child process ended
//// It was sent by a sigqueue, sending process (PID 1778)
//// Additional value: Zoli, 2014
